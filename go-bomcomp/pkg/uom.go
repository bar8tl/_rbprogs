// uom.go [2012-04-12 BAR8TL]
// Load needed unit of measures and conversion between some of them for 
// comparing quantities withing BOM comparison process
package bomcomp
...
func (u *Uom) NewUom(fname string) {
  ...
  rlen := len(u.Line[0].Fuom) + len(u.Line[0].Tuom) +
    int(unsafe.Sizeof(u.Line[0].Den)) + int(unsafe.Sizeof(u.Line[0].Num))
  f, err := os.Open(fname)
  if err != nil {
...
func (t *Umline) Uomdsp(fname string) {
  ...
  lrow := len(t.Fuom) + len(t.Tuom) + int(unsafe.Sizeof(t.Den)) +
    int(unsafe.Sizeof(t.Num))
